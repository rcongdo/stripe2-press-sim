# Canvas Scale, Edge Bleed & Registration Range Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Render the canvas at 4× internal resolution so halftone dots are invisible at 1× zoom (continuous-tone look) and become crisp at higher zooms; fix plate clipping so misaligned colors bleed visibly at artwork edges; extend registration range to ±4 mil.

**Architecture:** Add `SCALE = 4` to `PrintPreview.tsx` and multiply all coordinate constants by it — canvas attributes become 3680×1680 while CSS display dimensions stay `920 * zoom`. Bilinear downscaling at 1× makes 12px internal dots sub-pixel; at 4× zoom the canvas renders at native size, showing crisp dots. Replace per-zone `ctx.clip()` in `drawPlate` with a single pouch-level clip expanded by `4 * MIL_TO_PX` pixels so registration offset dots bleed visibly outside the artwork boundary. Registration nudge clamp in `ControlPanel.tsx` widens from ±2 to ±4 mil.
...
| File | Change |
|------|--------|
| `src/components/PrintPreview.tsx` | Add SCALE, scale all constants and drawing coords, update canvas attributes, remove per-zone clip in drawPlate, remove imageRendering toggle |
| `src/components/PrintPreview.test.tsx` | Update substrate fillRect assertion (920→3680); remove/replace imageRendering tests |
| `src/components/ControlPanel.tsx` | Change nudge clamp from ±2 to ±4 |
| `src/components/ControlPanel.test.tsx` | Update clamp test from 2 to 4 |

---

### Task 1: 4× internal canvas scale for continuous-tone appearance
...
| POUCH_H = 400 | 1600 |
| POUCH_TOP = 10 | 40 |
| POUCH_ORIGINS = [10,310,610] | [40,1240,2440] |
| MIL_TO_PX = 4 | 16 |
| PITCH = 12 | 12 (unchanged — 12px internal = 3px at 1× display) |

**Zone y-coords and heights (×4):**

| Zone | y_internal | h_internal |
|------|-----------|-----------|
| Header | 40 | 240 |
| Graphic | 280 | 880 |
| Flavor | 1160 | 200 |
...
```
/Users/robertcongdon/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node /Users/robertcongdon/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/npm run test -- --run 2>&1 | tail -6
```
Expected: `33 passed`. Stop and report BLOCKED if any fail.

- [ ] **Step 2: Write the complete replacement `src/components/PrintPreview.tsx`**
...
- [ ] **Step 3: Update `src/components/PrintPreview.test.tsx`**

Three changes to the existing test file:

**3a.** Update the substrate fillRect assertion — the canvas is now 3680×1680:
```ts
// Old:
...
**3b.** Remove the two `imageRendering` tests — they no longer apply because `imageRendering` is no longer set on the canvas at any zoom level (4× native scale makes it unnecessary):
- Delete: `"applies pixelated image rendering at zoom > 1"`
- Delete: `"does not apply pixelated image rendering at 1× zoom"`

**3c.** Add a replacement test confirming imageRendering is never set:
```ts
...
The canvas CSS width tests (`920px`, `1840px`, `460px`, `3680px`) are unchanged — they use `W * zoom = 920 * zoom`, which is the logical display size and has not changed.

- [ ] **Step 4: Run tests — expect 32 passing (33 - 2 removed + 1 added)**
...
If TypeScript errors appear, run:
```
/Users/robertcongdon/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node ./node_modules/.bin/tsc --noEmit 2>&1 | head -30
```
...
git -C /Users/robertcongdon/Documents/Codex/2026-05-12/i-want-to-build-a-flexographic \
  commit -am "feat: 4x internal canvas scale for continuous-tone look at 1x zoom"
```

---

### Task 2: Registration ±4 mil + per-pouch plate clipping

**Files:**
- Modify: `src/components/ControlPanel.tsx` (one line — nudge clamp)
- Test: `src/components/ControlPanel.test.tsx` (update clamp assertion)

**Note:** `drawPlate` already has the per-pouch clip from Task 1's complete file replacement above. This task only covers the ControlPanel changes.

- [ ] **Step 1: Read `src/components/ControlPanel.test.tsx` to find the existing clamp test**

Look for a test that calls nudge and checks that values are clamped at 2 or -2. Note the exact test name and assertion values — you'll update them.

- [ ] **Step 2: Update the clamp test in `src/components/ControlPanel.test.tsx`**

Find the test that asserts the nudge clamps at ±2. Update it to assert ±4. The test should look like this after the change:

```ts
it("clamps registration nudge at ±4 mil", () => {
  // start at +4, nudge right — should stay at 4
  const onRegistrationChange = vi.fn();
  render(
    <ControlPanel
      job={starterJob}
      settings={{ ...defaultSettings, registration: { ...defaultSettings.registration, cyanX: 4 } }}
...
If the existing test name mentions "2" or "±2", rename it to "±4". If the test uses `cyanX: 2` as the starting value and nudges to check clamping at 2, change it to `cyanX: 4`.

- [ ] **Step 3: Run tests to verify the clamp test now fails (the implementation still clamps at ±2)**

```
/Users/robertcongdon/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node /Users/robertcongdon/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/npm run test -- --run --reporter=verbose 2>&1 | grep -E "FAIL|pass|fail" | head -20
```
Expected: the clamp test fails; all others pass.

- [ ] **Step 4: Update the nudge clamp in `src/components/ControlPanel.tsx`**

Find line (currently line 54):
```ts
...
- [ ] **Step 5: Run all tests — expect 32 passing**
...
```bash
git -C /Users/robertcongdon/Documents/Codex/2026-05-12/i-want-to-build-a-flexographic \
  commit -am "feat: registration range ±4 mil; per-pouch plate clipping for edge bleed"
```
