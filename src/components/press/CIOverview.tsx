    ...
    <div className="press-overview" data-testid="press-overview" style={{ position: "relative" }}>
      ...
      <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} xmlns="http://www.w3.org/2000/svg">
        ...
        <path
          ...
          fill="none" stroke="#ccc" strokeWidth="5"
        />

        {/* Web lines to unwind/rewind (dashed, above the drum) */}
        <line x1={entry.x} y1={entry.y} x2={UNWIND_X} y2={UNWIND_Y + sag}
          stroke="#888" strokeWidth="2.5" strokeDasharray="5,3" />
        <line x1={exit.x} y1={exit.y} x2={REWIND_X} y2={REWIND_Y + sag}
          stroke="#888" strokeWidth="2.5" strokeDasharray="5,3" />
        <text x={UNWIND_X + 5} y={UNWIND_Y - 4} textAnchor="start" fontSize="9" fill="#999">Unwind</text>
        <text x={REWIND_X - 5} y={REWIND_Y - 4} textAnchor="end" fontSize="9" fill="#999">Rewind</text>

        {/* CI Drum */}
        <g
          onMouseEnter={mode === "learn" ? () => handleLabel("ciDrum") : undefined}
          onMouseLeave={mode === "learn" ? () => setTooltip(null) : undefined}
          style={mode === "learn" ? { cursor: "help" } : undefined}
        >
          <circle cx={DRUM_CX} cy={DRUM_CY} r={DRUM_R} fill="#2a2a4a" stroke="#555" strokeWidth="2" />
          <text x={DRUM_CX} y={DRUM_CY + 5} textAnchor="middle" fontSize="12" fill="#999" fontWeight="600">
            CI Drum
          </text>
        </g>
        ...
        {(STATION_ANGLES as readonly number[]).map((deg, slotIndex) => {
          ...
          const plateFill = active ? ch.displayColor + "44" : "#2a2a3a";
          const plateStroke = active ? ch.displayColor : "#555";
          const label = active
          ...
          return (
            ...
            >
              ...
              {/* Plate cylinder — hover triggers learn tooltip */}
              <g
                onMouseEnter={mode === "learn" && active ? (e) => { e.stopPropagation(); handleLabel("plateCylinder"); } : undefined}
                onMouseLeave={mode === "learn" ? () => setTooltip(null) : undefined}
                style={mode === "learn" && active ? { cursor: "help" } : undefined}
              >
                <circle
                  cx={DRUM_CX + PLATE_DIST} cy={DRUM_CY}
                  r={PLATE_R}
              ...
              {/* Anilox roll + ink chamber — hover triggers learn tooltip */}
              <g
                onMouseEnter={mode === "learn" && active ? (e) => { e.stopPropagation(); handleLabel("aniloxRoll"); } : undefined}
                onMouseLeave={mode === "learn" ? () => setTooltip(null) : undefined}
                style={mode === "learn" && active ? { cursor: "help" } : undefined}
              >
                <circle
                  ...
                  r={ANILOX_R}
                  fill={active ? "#2e2e4e" : "#222233"}
                  stroke={active ? "#666" : "#444"}
                  strokeWidth="1"
                />
                <rect
                  x={DRUM_CX + ANILOX_DIST + ANILOX_R + 3} y={DRUM_CY - 9}
                  width={20} height={18} rx="2"
                  fill={active ? ch.displayColor + "22" : "#1e1e3e"}
                  stroke={active ? "#666" : "#444"}
                  strokeWidth="1"
                />
              </g>
